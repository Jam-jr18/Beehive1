...
app.post('/api/orders', asyncHandler(async (req, res) => {
  ...
  await pool.execute(
    ...
    [id, customerName, tableNumber, JSON.stringify(items), total, orderType, paymentMethod, paymentReference, paymentSender, timestamp, 'Pending']
  );
...
app.post('/api/settings', asyncHandler(async (req, res) => {
  const { eWalletNumber, qrCodeUrl } = req.body;
  await pool.execute('DELETE FROM settings');
  await pool.execute('INSERT INTO settings (eWalletNumber, qrCodeUrl) VALUES (?, ?)', [eWalletNumber, qrCodeUrl]);
  res.json({ success: true });
}));
